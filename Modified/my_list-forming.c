...
void * producer_thread( void *arg)
{
    ...
    /* Local list for nodes to be appended before global list */
    ...
    while( counter  < K )
    {
        ...
        if( NULL != ptr )
        {
            ...
            {
                myLocalList->header = myLocalList->tail = ptr;
            }
            else
            {
                myLocalList->tail->next = ptr;
                myLocalList->tail = ptr;
            }
            ++counter;
        }
    }

    /* append the local list to the global list */
    while ( 1 )
    {
